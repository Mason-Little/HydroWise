...
Existing course tree (use courseCode, name, and topicName exactly as shown — do not invent values):
${treeJson}
...
Shape A — course matched:
{
  ...
  "chapter": { "pick": "match", "name": "<exact name from tree>" }
           OR { "pick": "create", "chapter": { "name": "<new name>", "description": "<new description>" } },
...
3. For chapter: find the best-matching chapter under the matched course. If one fits, use pick "match" with the exact name. If none fit, use pick "create" with a new name and description.
