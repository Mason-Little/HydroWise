export type LanguageModelTier = "tiny" | "fast" | "balanced" | "high" | "max";

export type LanguageModelId =
  | "qwen3.5-0.8b"
  | "qwen3.5-2b"
  | "qwen3.5-4b"
  | "qwen3.5-9b"
  | "qwen3.5-27b";

export type WebLanguageModelSource = {
  enabled: boolean;
  modelId?: string;
};

export type DesktopLanguageModelSource = {
  enabled: boolean;
  alias: string;
  hfRepo: string;
  hfFile: string;
  relativePath: string;
};

export type LanguageModelDefinition = {
  id: LanguageModelId;
  tier: LanguageModelTier;
  label: string;
  description: string;
  sortOrder: number;
  web: WebLanguageModelSource;
  desktop: DesktopLanguageModelSource;
};

export const DEFAULT_WEB_LANGUAGE_MODEL_ID: LanguageModelId = "qwen3.5-4b";
export const DEFAULT_DESKTOP_LANGUAGE_MODEL_ID: LanguageModelId = "qwen3.5-9b";

export const LANGUAGE_MODELS = [
  {
    id: "qwen3.5-0.8b",
    tier: "tiny",
    label: "Tiny",
    description: "Fastest option for simple questions and quick study help.",
    sortOrder: 0,
    web: {
      enabled: true,
      modelId: "onnx-community/Qwen3.5-0.8B-ONNX",
    },
    desktop: {
      enabled: true,
      alias: "qwen3.5-0.8b",
      hfRepo: "bartowski/Qwen_Qwen3.5-0.8B-GGUF",
      hfFile: "Qwen_Qwen3.5-0.8B-Q4_K_M.gguf",
      relativePath: "language/qwen3.5-0.8b/model.gguf",
    },
  },
  {
    id: "qwen3.5-2b",
    tier: "fast",
    label: "Fast",
    description: "Quick and reliable for everyday studying.",
    sortOrder: 1,
    web: {
      enabled: true,
      modelId: "onnx-community/Qwen3.5-2B-ONNX",
    },
    desktop: {
      enabled: true,
      alias: "qwen3.5-2b",
      hfRepo: "bartowski/Qwen_Qwen3.5-2B-GGUF",
      hfFile: "Qwen_Qwen3.5-2B-Q4_K_M.gguf",
      relativePath: "language/qwen3.5-2b/model.gguf",
    },
  },
  {
    id: "qwen3.5-4b",
    tier: "balanced",
    label: "Balanced",
    description: "Best mix of speed, clarity, and study quality.",
    sortOrder: 2,
    web: {
      enabled: true,
      modelId: "onnx-community/Qwen3.5-4B-ONNX",
    },
    desktop: {
      enabled: true,
      alias: "qwen3.5-4b",
      hfRepo: "bartowski/Qwen_Qwen3.5-4B-GGUF",
      hfFile: "Qwen_Qwen3.5-4B-Q4_K_M.gguf",
      relativePath: "language/qwen3.5-4b/model.gguf",
    },
  },
  {
    id: "qwen3.5-9b",
    tier: "high",
    label: "High",
    description:
      "Better for harder topics, deeper explanations, and tougher questions.",
    sortOrder: 3,
    web: {
      enabled: false,
    },
    desktop: {
      enabled: true,
      alias: "qwen3.5-9b",
      hfRepo: "bartowski/Qwen_Qwen3.5-9B-GGUF",
      hfFile: "Qwen_Qwen3.5-9B-Q4_K_M.gguf",
      relativePath: "language/qwen3.5-9b/model.gguf",
    },
  },
  {
    id: "qwen3.5-27b",
    tier: "max",
    label: "Max",
    description: "Best quality for the most demanding study sessions.",
    sortOrder: 4,
    web: {
      enabled: false,
    },
    desktop: {
      enabled: true,
      alias: "qwen3.5-27b",
      hfRepo: "bartowski/Qwen_Qwen3.5-27B-GGUF",
      hfFile: "Qwen_Qwen3.5-27B-Q4_K_M.gguf",
      relativePath: "language/qwen3.5-27b/model.gguf",
    },
  },
] as const satisfies readonly LanguageModelDefinition[];
