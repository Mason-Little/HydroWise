...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full flex-1 flex-col bg-background md:peer-data-[variant=inset]:m-1.5 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-lg md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-1.5",
        className,
      )}
      {...props}
    />
...
function SidebarInput({
  ...
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "h-7 w-full border-input bg-muted/20 dark:bg-muted/30",
        className,
      )}
      {...props}
    />
...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn(
        "m-1 flex flex-col justify-center gap-1.5 p-1.5 group-data-[collapsible=icon]:items-center",
        className,
      )}
      {...props}
    />
...
function SidebarGroupLabel({
  ...
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          "flex h-7 shrink-0 items-center rounded-md px-2 text-xs text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-7 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
          className,
        ),
      },
      props,
...
function SidebarGroupAction({
  ...
  return useRender({
    ...
    props: mergeProps<"button">(
      {
        className: cn(
          "absolute top-3 right-2.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-transform group-data-[collapsible=icon]:hidden after:absolute after:-inset-2 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 md:after:hidden [&>svg]:size-4 [&>svg]:shrink-0",
          className,
        ),
      },
      props,
  ...
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-1.5 text-left text-xs ring-sidebar-ring outline-hidden transition-[width,height,padding] transition-colors group-has-data-[sidebar=menu-action]/menu-item:pr-8 bg-transparent text-muted-foreground hover:bg-sidebar-accent/60 hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:text-sidebar-foreground data-active:font-medium data-active:shadow-sm group-data-[collapsible=icon]:!h-[46px] group-data-[collapsible=icon]:!w-[46px] group-data-[collapsible=icon]:!rounded-[11px] group-data-[collapsible=icon]:!p-0 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:text-[color-mix(in_srgb,var(--app-text-muted)_78%,var(--app-text-primary))] group-data-[collapsible=icon]:hover:!bg-[color-mix(in_srgb,var(--app-surface-primary)_18%,transparent)] group-data-[collapsible=icon]:hover:!text-[var(--app-text-primary)] group-data-[collapsible=icon]:data-active:!bg-[var(--sidebar-active-bg)] group-data-[collapsible=icon]:data-active:!text-[var(--sidebar-active-icon)] group-data-[collapsible=icon]:data-active:!font-normal group-data-[collapsible=icon]:data-active:!shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--app-border-solid)_26%,transparent)] group-data-[collapsible=icon]:data-active:!ring-0 group-data-[state=expanded]:justify-start group-data-[state=expanded]:px-2.5 focus-visible:ring-2 group-data-[collapsible=icon]:active:!bg-[color-mix(in_srgb,var(--app-surface-primary)_28%,transparent)] group-data-[collapsible=icon]:active:!text-[var(--app-text-primary)] active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent/60 data-open:hover:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 group-data-[collapsible=icon]:[&_svg]:size-[22px] [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
        ...
        outline:
          "bg-background shadow-[0_0_0_1px_hsl(var(--sidebar-border))] hover:bg-sidebar-accent/60 hover:text-sidebar-accent-foreground hover:shadow-[0_0_0_1px_hsl(var(--sidebar-accent))]",
      },
      size: {
...
function SidebarMenuAction({
  ...
  return useRender({
    ...
    props: mergeProps<"button">(
      {
        className: cn(
          "absolute top-1 right-1 flex aspect-square w-5 items-center justify-center rounded-[calc(var(--radius-sm)-2px)] p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-transform group-data-[collapsible=icon]:hidden peer-hover/menu-button:text-sidebar-accent-foreground peer-data-[size=default]/menu-button:top-1 peer-data-[size=lg]/menu-button:top-2 peer-data-[size=sm]/menu-button:top-0.5 after:absolute after:-inset-2 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 md:after:hidden [&>svg]:size-4 [&>svg]:shrink-0",
          showOnHover &&
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "mx-3 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border px-2 py-0.5 group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
    />
